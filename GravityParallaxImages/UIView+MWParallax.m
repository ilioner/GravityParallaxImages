...
- (void)setViewParallaxIntensity:(CGFloat)viewParallaxIntensity
{
    if (self.viewParallaxIntensity == viewParallaxIntensity)
        return;
    
    if (viewParallaxIntensity == 0.0) {
        [self endParallaxUpdates];
    } else if (self.viewParallaxIntensity == 0.0) {
        [self beginParallaxUpdates];
    }
    
    objc_setAssociatedObject(self, (__bridge const void *)(kMLDWRParallaxDepthKey), @(viewParallaxIntensity), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

-(CGFloat)viewParallaxIntensity
{
...
- (void)motionsUpdated
{
    CGFloat parallaxIntensity = self.viewParallaxIntensity;
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        CGPoint attitudeDifference = CGPointMake(self.mw_attitudeDifference.x * self.viewParallaxIntensity, self.mw_attitudeDifference.y * self.viewParallaxIntensity);
        CGAffineTransform newTransform;
        if (self.viewParallaxIntensity > 0) {
